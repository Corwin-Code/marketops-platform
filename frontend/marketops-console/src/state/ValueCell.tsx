 ...
 *
 * The qualifier is a visible tag beside the value rather than only a tooltip or
 * a colour, because an operator scanning a list reads text and does not hover.
 * The tone is also on the element as data, so a browser test can assert that
 * nothing uncertain was ever rendered as confirmed without depending on styling.
 */
export function ValueCell({
  ...
  const absent = presentation.tone === 'absent';
  return (
    <div data-tone={presentation.tone} data-label={label}>
      <Card size="small">
        <Flex vertical gap={4}>
          <Typography.Text type="secondary" ellipsis={{ tooltip: label }}>
            {label}
          </Typography.Text>
          <Typography.Text
            strong={!absent}
            {...(absent ? { type: 'secondary' as const } : {})}
            style={{ fontSize: 20, fontVariantNumeric: 'tabular-nums' }}
          >
            {absent ? '—' : formatAmount(value, currencyCode)}
          </Typography.Text>
          <Flex gap={8} align="center" wrap>
            <ConfidenceTag
              valueState={valueState}
              {...(confidenceState === undefined ? {} : { confidenceState })}
            />
