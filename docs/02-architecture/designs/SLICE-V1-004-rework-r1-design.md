...
等价 OFFICIAL_SUMMARY 的聚合方法输入（Final Closure 续接）：关键群组只有作为同来源同窗口分层的可实现子集才进入方法桥，即对每个来源分层 n ≤ N、k ≤ K 且 K − k ≤ N − n（补集必须能承担剩余成功）；两个单独上界不足以证明子集关系。群组允许重叠，不要求全部群组求和等于来源总计。不可实现群组按既有同群体规则拒绝并原样留存，来源分层与独立总值资格不受影响，受影响保护为 UNDETERMINED 而非 PASS。

Required evidence:

- 正常角色/来源入口证明独立DETAIL、独立等价SUMMARY及完整0购买均按定义计算。
- 不完整窗口、错summaryKind、重放/冲突分子、超界分子必须明确限用途，不伪造值。
- 不可实现补集群组不得取得合格群组比较或保护 PASS；合法补集边界、重叠群组与带合格聚合输入的 SUMMARY 晚事实经重算修订正式 Outcome 一次。
...
## Exact restoration through the existing business action chain (implementation boundary)

S4-DR-R1-020 requires a new action, not a reversal transition on the original command. Extend the existing description preparation with an optional immutable source-command reference. For restoration, the target is exactly that source's captured nonempty prior text; its organization/listing and captured current version must match. The new action uses the existing preparation, independent review, current approval, binding, allowance and atomic launch route. Review/Console must show the restoration source and exact target. `COMMAND_RESOLVE` alone must have no route to a RESTORE dispatch. Existing historical attempts and completed compensation evidence remain preserved.

Use the sole description Command/Outbox for the new action. Its RESTORE attempt writes the new command's approved target, with its own idempotency identity and current approval. Before dispatch, a fresh readback at the current fence must match that new command's captured prior version and provide the verified conditional-write token. A later legitimate version, missing/empty prior target, missing native precondition support, wrong/newly expired approval, or unresolved original native effect prevents dispatch. An accepted asynchronous restoration task follows the existing read-only polling/custody path; technical retries do not manufacture another business approval. Its final native/readback evidence qualifies the new restoration action only.

Keep this change within the existing owners and protocol machinery. Do not introduce a second approval service, compensation queue or provider-specific implementation. The current canonical description observation remains a preparation prerequisite, obtainable through the existing fact/acquisition path; do not label manually supplied description facts as official readbacks. The original recommendation may remain under outcome observation, so the new exact restoration may exclude only its proven source recommendation from the duplicate-live-proposal check; all other live conflicts and the database's one-live-action constraint remain enforced. Ordinary product-purpose, materiality, allowance and Outcome defects remain tracked under their own frozen roots and may not be bypassed merely to obtain a restoration-positive test.

Required connected acceptance for this change: no-new-approval and wrong-object refusal; old approval expiry with a valid new approval; current-version conflict; unsupported empty/missing source; exact conditional synchronous restore; asynchronous pending/final and post-dispatch loss without duplicate mutation. No closure from permanently disabling restoration. The prior connected fixtures provide the transport/custody harness; expand that harness only for these required behaviors.

Implementation notes for V0097: the existing live-recommendation index is retained for ordinary work. Only an exact restoration proposal is separated; a second partial unique index plus listing-row-locked validation prevents multiple restoration proposals and conflicting ordinary work, while allowing its proven source recommendation to keep its Outcome responsibility. Neither the source Action nor its occupations is closed to create artificial capacity. The connected positive fixture explicitly budgets for both occupations.

The new restoration reuses the existing Worker. First readback must match the new command's captured current value at its current fence, with a complete exact native response and the registry-defined conditional version. An attempt cannot reuse a preflight preceding a later mutation. The actual RESTORE body is the new approved target; its conditional header and own operation idempotency key are verified immediately before HTTP. Legacy compensation requests receive RESTORE_UNSUPPORTED, and historical pending compensation is routed to manual resolution without mutation. New restoration result qualification uses its own RESTORE completion and subsequent target readback. Async and post-dispatch loss use the existing task/raw/unknown machinery.

The normal connected chain exposed a missing execution evaluation after atomic launch replaced the second command-creation endpoint. ListingActionLaunchService now invokes the existing GuardrailService for EXECUTION inside the launch transaction before acquiring allowance. No new guardrail owner or approval channel is introduced. Pure bounded description request-schema validation is callable by the application role for the read-only gate; it has no table access or mutation authority.

Controlled clock diagnosis reproduced the earlier intermittent preparation refusal with the Action/Evaluation application clocks advanced by one minute. The unchanged V0088 chronology constraint correctly rejected the application-stamped future plan. Persisted listing preparation, plan freeze, review, listing approval decision and binding now use the database clock from existing repositories. Authentication age still uses the existing authentication clock checks; price/advertising decision timing is unchanged. Plan digest and decision horizon use the same database-derived instant that is persisted. There is no added tolerance, retrospective freeze or weakened chronology constraint. The controlled red/green probe is retained separately from the connected recovery scenarios.

### Cumulative allowance correction (frozen roots 012–013, in progress)

Use one Operations Workflow projection for preview and launch, under one organization transaction lock at acquisition. The accepted ALLOWANCE_AXES category declares the complete required set; ALLOWANCE_RESERVE declares its minimum disposal reserves. A legacy axis array permits only unambiguous single-scope resolution. An explicitly accepted object with `axes` and `scopeComposition: ALL_APPLICABLE` applies every matching scope constraint. No most-specific-row override is inferred. Missing axes, malformed policy, incompatible units and unqualified demand remain explicit gaps.

Usage follows outstanding Actions and their exact frozen affected sets in the constraint's real scope, across allowance configuration IDs and versions. Concurrent listings and affected variants use distinct canonical identities; repeated work on the same identity does not manufacture another listing, and releasing one of several outstanding obligations does not remove the others. Actual and Unknown occupations remain included regardless of old stored zero values. Launch records its canonical demand and the evaluated policy/constraint snapshot; request numbers have no authority. Revenue and category exposure require qualified canonical financial/demand facts and remain unresolved until those consumers are connected; this intermediate correction does not close root 012.

The legacy arbitrary occupation observation and generic observation-as-stop release routes must be removed. Positive release must bind the exact Action, axis, independent evidence and purpose, separating cessation of new exposure from clearing historical obligations. Plain old-text readback, Task closure, elapsed time and executor self-report prove neither. This remains root 013 work, not permission to permanently disable all positive release. Tests will first establish cross-version accumulation, complete axes, shared preview, request-tampering refusal and concurrent acquisition, then exercise the qualified release lifecycle.

Implementation self-check: acquisition takes the stable organization lock before rechecking current binding/Health/containment, then holds a SHARE lock on allowance configuration while consuming its exact projection. Concurrent launches may share the configuration lock; configuration changes cannot race the committed acquisition. Variant usage lacking a complete frozen identity stays unresolved. The removed Java preview arithmetic and unused repository counter have no replacement local calculator. The separate historical AllowanceCheck domain example is not used by either production preview or launch.

V0098 also removes the unsafe legacy release inference. Its currently qualified positive basis is an exact API command terminal before any admitted mutation attempt, without linked promotion obligations. The function verifies consumed proof actor/organization, serializes with acquisition and retains structured source evidence. No arbitrary numeric decrease remains callable by the application role. Ordinary observation is a rejection regression, replacing the old false-positive release test. Independent promotion cessation and historical clearing are the next connected work, not satisfied by this limited positive case.

### Promotion terms and lifecycle connection (roots 014–015 and 013)

First bind the existing promotion terms/obligations declaration to the Action while it is prepared. Reuse the current seven-field promotion envelope and its two existing candidate kinds; a PostgreSQL canonical digest enters the existing recommendation/approval identity. Retain the exact bounded strings, without silently normalizing native keys or commercial terms. An accepted Action cannot acquire a different declaration at entry or replace its declaration later. Old Actions retain NULL rather than acquiring invented historical terms. This freeze is an identity control, not evidence that the declared economics or platform status is qualified.

The full declaration may contain financial information. Its read projection must recheck current store and every frozen product's financial evidence access, including after preparation/review. The ordinary Action view carries its digest, and qualified reviewers retrieve the exact declaration. Existing manual packets must bind the same declaration rather than introducing another target. No promotion provider API, approval owner or queue is added.

Then connect the existing independent manual verification path to exact participation/exit observations, carrying purpose, actual executor, terms identity, observed time and residual obligations. Reported facts and qualification remain distinct. Qualified cessation of new exposure cannot clear historical obligations. Entry/exit authorization, existing external authorization for adoption and the shared economics/demand consumers remain part of this connected work; freezing a blob or disabling release does not close these roots.


V0099 implementation boundary: declaration booleans require explicit values; database-generated digest validation and immutable Action/linked-engagement triggers retain identity even across direct application writes. Full financial disclosure uses the frozen affected-product set, including current grant revocation. The existing entry service validates but no longer strips the bound evidence reference. This stage adds no approval lifecycle, provider endpoint, optimizer or queue. The observed participation/exit and residual-obligation evidence remain separate work; the original obligations map is the approved commitment, not a mutable release balance.


Promotion disclosure same-class correction (root 002): the existing engagement list/detail and all mutation return values use one current financial projection inside ManualPathService. Operational state remains visible under ordinary scope; commercial maps and their source reference require financial access. Linked Actions use every frozen product plus store. Adopted records without historical product lineage can be fully disclosed only to current organization-wide financial access; current store mapping is not substituted for unknown historical scope. This changes disclosure, not adoption qualification or write authority, and requires no migration. The actorless public detail read is removed.


### Exact independent promotion participation (roots 014/016)

Extend the existing fact intake, provenance, manual packet/report/verification and Action transition. V0100 adds one append-only promotion observation relation with exact native kind/key, source/ingestion time and optional complete declaration. Null declaration explicitly preserves unknown commercial terms; it cannot match an approved target. The authenticated human intake always records MANUAL_ENTRY and never upgrades a person's statement to marketplace Raw. A later complete or changed observation is another fact, not a replacement.

Manual participation qualification consumes the exact packet's frozen declaration and the executor's reported operation within that packet's authority interval. Observation identity and custody must match organization/listing/native promotion, occur after the operation and be acquired by verification time. A currently authorized independent human may verify independent human evidence; executor/reporters cannot provide their own independent observation, and a human source cannot be labelled official. The current full financial projection is required to verify commercial identity. Unknown/nonparticipating/different/incomplete declarations cannot produce MATCHED_TARGET. A promotion cannot borrow description/display observation IDs or claim MATCHED_PRIOR without a frozen promotion prior.
