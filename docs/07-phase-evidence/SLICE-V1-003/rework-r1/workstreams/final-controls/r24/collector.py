...
out=pathlib.Path('/tmp/slice3-r1-final-controls-r24');out.mkdir(exist_ok=False)
classes=['PriceCommandWorkerIT','AdvertisingOrchestrationCapacityIT','AdvertisingCaseCalculationServiceTest','AdPriorityLexicographicTest','AdvertisingEfficiencyFlowIT','AdvertisingReservationIT']
...
r={'kind':'TARGETED_W7_CI_PRECISION_AND_QUEUE_ISOLATION_REPAIR_R24','cwd':str(cwd),'command':argv,'baseHead':head,'baseTree':git('rev-parse','HEAD^{tree}'),'workingTreeStatusBefore':git('status','--porcelain=v1'),'sourceBinding':'Exact source manifests bind W7 published checkpoint plus explicit precision boundary and Price worker test isolation repairs. This run does not relabel the prior W7 CI failures or unchanged W6 measurements.','sourceBeforeSha256':sha(out/'source-before.json'),'resourcesSha256':sha(resource),'boundary':'Synthetic isolated PostgreSQL only; no real Provider/shared/production. production_write_enabled=false.','startedAtUTC':now()}
...
with (out/'maven.log').open('wb') as log:completed=subprocess.run(argv,cwd=cwd,stdout=log,stderr=subprocess.STDOUT)
