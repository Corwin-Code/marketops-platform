...
### R29 full canonical Outcome journey

`AdvertisingVerticalPathIT.acceptedFactsDriveRealHumanCommandFixtureReadbackAndMatureOutcomeHistory` now reaches all stages from genuine calculator inputs and a Planner-derived frozen baseline. Following actual fixture worker readback, fresh Completed/company and every critical unit produce early `UNCHANGED` safety and release the reservation. Current accepted reports, a Completed cohort revision, fresh settled fee/return publication and a nonoverlapping cost version are then recalculated by the real Metric engine. The 30-day Retained stage becomes `IMPROVED`; independently attributed actual settlement becomes `IMPROVED`. A later accepted financial correction produces `REGRESSED`, containment and one Finance secondary Task, while the preceding settlement observation remains `IMPROVED`; replay yields no extra revision.

This complements the explicit golden pre-action baselines used in `AdvertisingFrozenOutcomeIT` for precise fault isolation. It proves the trusted Planner and observed evidence pipeline together. R29's full narrow batch passed **34/34**, including both Vertical journeys and the three controlled-write suites; exact commands, per-suite outcomes and immutable log copies are in [facts-outcome-traceability.json](facts-outcome-traceability.json). The source remains pending final clean verification and exact published-head CI.
