...
INPUT=Path('/tmp/slice3-checkpoint-backend-ci-7e66cf8')
OUT=Path(__file__).resolve().parent/'publication-scan';assert not OUT.exists();OUT.mkdir()
VALIDATOR=Path(__file__).resolve().parent/'governance-source-7e66cf8.py'
...
write('SCAN-INPUT.json',{'kind':'ORIGINAL_OFFICIAL_CI_RAW_SCAN_INPUT','files':before,
    'sourceHead':'7e66cf87afc15773d4f6e6e6717a86e7b8336ae5','runId':34001582148,'runAttempt':1,
    'checkpointResult':'PRE_SECTION_6_24_REPAIR_SUCCESS_CHECKPOINT_NOT_FINAL_CLOSURE','officialArtifactsDigestVerified':True,
    'officialArtifacts':[{'id':r['id'],'name':r['name'],'bytes':r['size_in_bytes'],'digest':r['digest']} for r in artifacts],
    'rulesSource':{'path':'scripts/validate_governance.py at 7e66cf87afc15773d4f6e6e6717a86e7b8336ae5','sha256':sha(VALIDATOR.read_bytes())},
...
try:
    ...
    write('SCAN-SUMMARY.json',{'kind':'RAW_PUBLICATION_SHAPE_SCAN','status':status,'scope':'All original files plus actual ZIP/GZIP/TAR nested members; raw UTF-8 bytes and decoded XML/HTML/JSON strings.',
      ...
      'limitations':'A bounded pattern scan is not a universal no-PII proof. Every shape hit needs exact classification; JWT/test-token bytes remain private. Checkpoint remains historical before section 6.24 repair, not final closure.'})
