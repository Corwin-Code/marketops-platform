...
O=Path('/tmp/slice3-root-selected-evidence-e278b1e-r3');assert not O.exists();O.mkdir()
paths={'sourcePlan':Path('/tmp/slice3-publication-observation-e278b1e-r2/SOURCE-METHOD-REVIEW-PLAN.json'),'methods':Path('/tmp/slice3-method-proposals-e278b1e-r3/PROPOSED-METHOD-BINDINGS.json'),'structured':Path('/tmp/slice3-structured-proposals-e278b1e-r4/PROPOSED-STRUCTURED-BINDINGS.json'),'catalog':Path('/tmp/slice3-final-assessment-e278b1e-r2/CURRENT-NODE-CATALOG.json'),'slots':Path('/tmp/slice3-final-assessment-e278b1e-r2/ASSESSMENT-SLOTS.json'),'publicationReview':Path('/tmp/slice3-publication-observation-e278b1e-r2/ROOT-PUBLICATION-OBSERVATION-REVIEW.json'),'scopeReviews':Path('/tmp/slice3-structured-crosschecks-e278b1e-r1/ALL-STRUCTURED-SCOPE-REVIEWS.json')}
...
for section,count in [('criteria',200),('findings',22),('clauses',115),('verificationChecks',5)]:
 ...
 for slot in x['slots'][section]:
  ...
  publication_sets={
   'S3-DR-001:REQUIRED_REWORK:3':['capture-identity-and-original-attempts','measured-versus-containing-source','complete-original-backend-index','complete-seven-local-layer-archives','official-artifact-digests'],
   'S3-DR-001:VERIFICATION:2':['exact-pr','exact-all-contexts','capture-identity-and-original-attempts'],
   'S3-DR-001:VERIFICATION:3':['complete-original-backend-index','complete-seven-local-layer-archives','official-artifact-digests','capture-identity-and-original-attempts']}
  if slot['id'] in publication_sets:
   assert not selections and section=='clauses'
   for name in publication_sets[slot['id']]:
    found=[n for n in nodes.values() if n['layer']=='security' and n['name']=='publication-observation.'+name];assert len(found)==1
    n=found[0];selected_adapter=[a for a in x['publicationReview']['exactTypedAdapters'] if a['name']==n['name']];assert len(selected_adapter)==1 and selected_adapter[0]['assertions']==n['assertions'] and all(selected_adapter[0]['evidence'][k]==n['evidence'][k] for k in ['path','sha256']) and ref(n['evidence']['path'])==selected_adapter[0]['evidence']
    selections.append({'nodeId':n['nodeId'],'role':'supporting','scope':s['engineeringReason']+' Typed original observation: '+name+'. '+x['publicationReview']['boundary'],'origin':'SEPARATELY_COLLECTED_GITHUB_OR_ARCHIVE_OBSERVATION','rootReview':ref(paths['publicationReview'])})
  unique={}
  ...
  if slot['id'] in publication_sets:reason=s['engineeringReason']+' '+x['publicationReview']['boundary']
