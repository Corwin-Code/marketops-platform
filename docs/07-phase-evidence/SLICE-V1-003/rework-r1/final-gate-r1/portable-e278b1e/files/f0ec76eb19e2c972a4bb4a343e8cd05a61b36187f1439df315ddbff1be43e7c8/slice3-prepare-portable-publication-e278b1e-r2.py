...
class Plan:
    ...
    def histories(self):
        ...
        for p in ['/tmp/slice3-backend-checkpoint-archives-r1/7e66cf8-INDEX.json','/tmp/slice3-backend-checkpoint-archives-r1/98344bb-INDEX.json','/tmp/slice3-backend-checkpoint-archive-02e6172-r5/02e6172-INDEX.json','/tmp/slice3-backend-checkpoint-archive-da559bd-r6/da559bd-INDEX.json']:
...
def main():
    ...
    admission=Path('/tmp/slice3-root-admission-e278b1e-r1')
    plan.directory(admission,'ROOT_FINAL_ENGINEERING_ADMISSION_WITH_EXPLICIT_CONTROLLER_BOUNDARY')
    plan.add('/tmp/slice3-prepare-root-admission-e278b1e-r1.py','ACTUAL_ROOT_ADMISSION_BUILDER')
    for row in read(admission/'REQUIRED-DIRECT-FILES.json')['files']:
        actual=plan.add(checked(row['original'],admission),'ADDITIONAL_FULL_PARENT_OR_ROOT_FINALIZER_INPUT',row['original']['sha256'])
        need(actual['repositoryPath']==row['repositoryPath'],'ROOT_ADMISSION_DESTINATION_DIFFERS')
    plan.histories()
