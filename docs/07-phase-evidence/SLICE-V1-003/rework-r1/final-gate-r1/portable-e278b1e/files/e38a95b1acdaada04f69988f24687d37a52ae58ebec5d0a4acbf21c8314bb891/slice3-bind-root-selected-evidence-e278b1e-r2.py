...
O=Path('/tmp/slice3-root-selected-evidence-e278b1e-r2');assert not O.exists();O.mkdir()
 ...
 for slot in x['slots'][section]:
  ...
  if slot['id'] in publication_sets:
   ...
   for name in publication_sets[slot['id']]:
    ...
    n=found[0];selected_adapter=[a for a in x['publicationReview']['exactTypedAdapters'] if a['name']==n['name']];assert len(selected_adapter)==1 and selected_adapter[0]['assertions']==n['assertions'] and selected_adapter[0]['evidence']==n['evidence']
