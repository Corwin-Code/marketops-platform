...
parser=argparse.ArgumentParser();parser.add_argument('--input',required=True);parser.add_argument('--out',required=True);parser.add_argument('--receipt',default='layer-candidate.json');parser.add_argument('--scope',required=True);parser.add_argument('--expected-result',required=True);args=parser.parse_args()
...
VALIDATOR=Path(__file__).resolve().parent/'governance-source-02e6172.py'
assert hashlib.sha256(VALIDATOR.read_bytes()).hexdigest()=='89f716bbada69c90db33869b7d15ba5e702b595f07d4ac690373ebbadff60b34'
...
assert INPUT.is_relative_to(Path('/tmp').resolve())
assert not any(p.is_symlink() for p in INPUT.rglob('*')),'Unexpected symlink in frozen input'
...
layer=json.loads((INPUT/args.receipt).read_text())
assert layer['sourceHead']=='02e617278793b4458dfb7cac74c2a937f1dfcb00'
assert layer['sourceTree']=='8d8dd0b3429797a0e01b2ff91c1ba5c09de17dcc'
assert layer['sourceInventorySha256']=='9706e844337851329c5e30954598162ac52144c82facaf736b04475c25f0c6ec'
assert layer['sourceStable'] is True and layer.get('finishedAt') and layer['result']==args.expected_result
assert (layer['exitCode']==0)==(args.expected_result=='COMMAND_SUCCEEDED_REVIEW_REQUIRED')
...
write('SCAN-INPUT.json',{'kind':'ORIGINAL_LOCAL_LAYER_RAW_SCAN_INPUT','files':before,
    ...
    'sourceStable':layer['sourceStable'],'layerReceipt':{'path':str(INPUT/args.receipt),'sha256':sha((INPUT/args.receipt).read_bytes())},
    'allCollectorEvidenceReferencesVerified':len(layer['evidence']),
    'scanner':{'path':str(Path(__file__).resolve()),'sha256':sha(Path(__file__).read_bytes())},
    'receiptSchema':args.receipt,'rulesSource':{'path':'scripts/validate_governance.py','sourceHead':layer['sourceHead'],'archivedSource':str(VALIDATOR),'sha256':sha(VALIDATOR.read_bytes())},
    'patterns':[{'id':i+1,'pattern':p} for i,p in enumerate(rules)],'rawContentPrinted':False,
    'checkpointBoundary':args.scope,'checkpointExitCode':layer['exitCode'],'originalCollectorTestcaseCounts':layer.get('testcaseCounts'),
    'sourceReceiptAuthority':'Original immutable local command receipt; scan does not convert command completion into nine-layer admission.'})
...
try:
    ...
    after_files=sorted(p for p in INPUT.rglob('*') if p.is_file())
    after={str(p.relative_to(INPUT)):{'bytes':p.stat().st_size,'sha256':sha(p.read_bytes())} for p in after_files}
    if before!=after:status='INVALID_ORIGINAL_BYTES_OR_FILESET_CHANGED'
    write('SCAN-SUMMARY.json',{'kind':'RAW_PUBLICATION_SHAPE_SCAN','status':status,'layer':INPUT.name,'checkpointBoundary':args.scope,'originalCommandResult':layer['result'],'originalCommandExitCode':layer['exitCode'],'sourceHead':layer['sourceHead'],'sourceTree':layer['sourceTree'],'runId':layer['runId'],'scope':'All physical layer files plus actual ZIP/GZIP/TAR nested members; raw UTF-8 bytes and decoded XML/HTML/JSON strings. Image bytes are scanned without OCR or pixel inspection.',
      ...
      'limitations':'A bounded pattern scan is not a universal no-PII proof. Every shape hit needs exact classification; JWT/test-token bytes remain private. Each layer retains its own original execution identity and success/failure boundary. Image pixels were not OCR-reviewed; binary strings and actual archive members are scanned. No claim that zero shape hits proves absence of all secrets or PII.'})
