...
SCAN=Path('/tmp/slice3-browser-r4-evidence-02e6172-r1')
...
archives=[]
for name in ['browser-r4']:
 scan=SCAN/'scan';inventoryPath=scan/'ORIGINAL-FILE-SHA-INDEX.json';inv=json.loads(inventoryPath.read_text());scanReceiptPath=scan/'PUBLICATION-SCAN-RECEIPT.json';scanReceipt=json.loads(scanReceiptPath.read_text())
 ...
 assert sorted(p.relative_to(root).as_posix() for p in paths)==[r['relativePath'] for r in inv['files']]
 rows=[]
 for r in inv['files']:
 ...
 archivePreexisted=archive.exists()
 if not archivePreexisted:
