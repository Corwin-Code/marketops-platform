...
P=pathlib.Path;scan=P('/tmp/slice3-historical-1381-raw-scan-r1');orig=P('/tmp/slice3-final-execution-1381ef7-r4/backend-full');out=P('/tmp/slice3-historical-1381-publication-package-r1/triage');out.mkdir(exist_ok=False)
...
summary=read(scan/'SCAN-SUMMARY.json');scanInput=read(scan/'SCAN-INPUT.json');parent=read(orig/'layer-candidate.json');hits=[json.loads(s)for s in (scan/'HITS.jsonl').read_text().splitlines()];members={r['path']:r for r in map(json.loads,(scan/'MEMBERS.jsonl').read_text().splitlines())};patterns={x['id']:re.compile(x['pattern'])for x in scanInput['patterns']}
