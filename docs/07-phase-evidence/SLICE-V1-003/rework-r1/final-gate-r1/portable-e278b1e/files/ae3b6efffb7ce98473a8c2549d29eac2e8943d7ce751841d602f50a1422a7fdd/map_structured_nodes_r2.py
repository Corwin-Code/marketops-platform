...
for m in rules['matches']:
 ...
 if rules.get('derivationSourceIdentity')is not None:
  ...
  assert refEq(n['evidence'],layer['receipt'])or actual_reference(parent),'Exact structured raw is neither the actual registered execution receipt nor its registered member'
