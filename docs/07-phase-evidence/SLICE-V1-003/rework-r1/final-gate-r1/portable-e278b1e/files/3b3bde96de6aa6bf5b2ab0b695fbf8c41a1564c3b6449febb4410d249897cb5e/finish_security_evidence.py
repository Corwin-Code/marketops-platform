...
MERGE='f9a1d73f324323d13ea72db3890090c9b9d85450'
...
current={a['number']:a for page in load('dismissed-alerts-pages.json') for a in page}
...
for number,a in sorted(current.items()):
 ...
 rows.append(row)
write('HISTORICAL-DISMISSED-HIGH-RECONCILIATION.json',{'kind':'SEPARATE_HISTORICAL_DISMISSED_HIGH_RECONCILIATION','sourceHead':HEAD,'testedMerge':MERGE,'priorEvidence':proof(oldpath),'currentRawEvidence':proof(OUT/'dismissed-alerts-pages.json'),'count':len(rows),'newDismissalsPerformed':False,'findings':rows})
...
node=next(n for n in ast.parse(validator.read_text()).body if isinstance(n,ast.Assign) and any(isinstance(t,ast.Name) and t.id=='SECRET_PATTERNS' for t in n.targets))
