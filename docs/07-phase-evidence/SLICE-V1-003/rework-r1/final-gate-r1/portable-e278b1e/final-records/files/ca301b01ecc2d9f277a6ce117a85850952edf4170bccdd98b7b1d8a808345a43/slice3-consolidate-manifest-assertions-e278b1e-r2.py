...
R=Path('/Users/chzhengx/Code/personal/marketops-platform');O=Path('/tmp/slice3-conjunction-admission-e278b1e-r2');assert not O.exists();O.mkdir();P=Path('/tmp/slice3-root-admission-e278b1e-r1/EXECUTION-MANIFEST.json');original=json.loads(P.read_text());m=json.loads(P.read_text());audit=[]
  ...
  for items in groups.values():
   ...
   if len(items)==1:q=p
    ...
    for _,atom in items:
     ...
     scopes.append({k:atom[k] for k in ['name','scope','assertions'] if k in atom})
