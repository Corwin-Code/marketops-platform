...
| `ci-checks.md` | OWNER_RULESET_ACTION_REQUIRED | All eleven stable jobs passed; PR/security evidence is current, while the Ruleset still requires only one context |
| `traceability.md` | VERIFIED | Each foundation contract names its implementation and a test that detects removal |
| `unavailable-capabilities.md` | OWNER_RULESET_ACTION_REQUIRED | Technical capability gaps are resolved; the remaining boundary is Owner-only Ruleset authority |
