...
| --- | --- | --- | --- |
| BC-01 | Temporal boundary completeness: closed kind set, missing/duplicate/unexpected kind fail closed, `valid_until` from a counted relation, no `LEAST(NULL)` assumption, grant strictly before the boundary, authority capped | `V0009`, `V0010`, `ControlBoundaryCompletenessIT` (TC-CTRL-200…209), `IngestionAuthorityAndEvidenceIT` (TC-CTRL-400/402/404/405) | `backend-verify-run.txt` |
