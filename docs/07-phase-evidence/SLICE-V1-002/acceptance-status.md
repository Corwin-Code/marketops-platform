...
The starting Draft PR #26 Head had all 12 required contexts and aggregate
CodeQL passing. After the final rework Head is published, the same required
contexts and aggregate CodeQL must pass again; the exact final readback is
bound in the external PR packet because this commit cannot contain its own
identity. No threshold changes.
...
| `S2-AC-063` | `EXECUTABLY_VERIFIED` | ordinary action SLA may pause only while exception governance and expiry remain active. | TC-CASE-016 and TC-CASE-018 assert persisted `original_action_due_at`, `action_sla_paused_at` and exact remaining duration, preserve the original due time while accepted, then rebase and clear the pause fields on invalidation/expiry without breaking journal continuity |
...
| `S2-AC-070` | `EXECUTABLY_VERIFIED` | CRITICAL targeted card/Task update meets P95 <= 5 minutes and hard <= 15 minutes from `fact_accepted_at`. | `RepresentativePerformanceIT` is the declared-capacity authority: 5,000 attributable canonical facts enter through `AvailabilityRecalculationScheduler.recalculateWhatChanged`, use the durable accepted-fact cursor and identity resolution/dedup, and complete evidence, calculation, projection, Case, automatic verification and SLO persistence in real PostgreSQL; the artifact measures CRITICAL P95 and hard maximum from the common `fact_accepted_at`. `TC-TARGET-CAP-001` is supporting mocked worker-accounting evidence only |
| `S2-AC-071` | `EXECUTABLY_VERIFIED` | HIGH/WATCH/blocker targeted update meets hard <= 15 minutes. | `RepresentativePerformanceIT` is the actual-path authority for the five-lane 5,000-Variant distribution and records each lane's hard maximum from `fact_accepted_at`; `TC-SLO-001` pins the bound semantics, while `TC-TARGET-CAP-001` is supporting mocked worker-accounting evidence only |
| `S2-AC-072` | `EXECUTABLY_VERIFIED` | full portfolio reconciliation succeeds at least hourly at the declared acceptance capacity. | `RepresentativePerformanceIT` is the actual PostgreSQL authority: it keyset-pages all 5,000 Variants, records the completed sweep and positive hourly margin, and repairs 50/50 deliberately dropped targeted triggers. `TC-RECON-003` is supporting mocked worker-paging evidence only |
...
| `S2-AC-095` | `EXECUTABLY_VERIFIED` | performance evidence proves the internal SLOs and hourly sweep at the declared acceptance capacity. | `RepresentativePerformanceIT` alone supplies the actual-path declared-capacity authority over 5,000 Variants in real PostgreSQL: canonical accepted facts traverse ingestion/cursor/resolution/dedup, targeted worker, evidence, calculation, projection, Case, automatic verification and SLO, followed by the real 5,000-Variant sweep and 50/50 recovery. Exact configuration, timings and trace counts are recorded in `target/performance/representative-v1.json`; `TC-TARGET-CAP-001` and `TC-RECON-003` remain supporting mocked worker-accounting/paging tests and are not runtime capacity authority |
...
| `S2-AC-098` | `EXECUTABLY_VERIFIED` | full repository regression, governance validation, production- readiness validation and security scans pass with no threshold weakening. | local backend/frontend/browser/governance/readiness/supply-chain gates pass without weakened thresholds; the final Draft PR #26 Head must repeat all 12 required contexts plus aggregate CodeQL, with exact results recorded in the external PR packet after publication |
