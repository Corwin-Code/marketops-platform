...
public class ListingTaskSloService implements ListingTaskSloQuery {
    ...
    public void bind(UUID taskId, UUID organization, UUID recommendation, Instant raisedAt,
        ...
        String digest=Digest.ofComponents(List.of("LC_TASK_RESPONSIBILITY_1",taskId.toString(),recommendation.toString(),
        ...
        jdbc.sql("""
                ...
                VALUES (:task,:org,:recommendation,:package,:version,:raised,CAST(:slo AS jsonb),CAST(:coverage AS jsonb),
                    :digest,:state,:ack,:action,:outcome,clock_timestamp())
                ...
                .param("action",timestamp(schedule.actionDueAt())).param("outcome",timestamp(schedule.outcomeMaturityDueAt())).update();
    }
    ...
    public Optional<Status> statusForRecommendation(UUID recommendationId, Instant asOf) {
        return jdbc.sql("""
                ...
                FROM ops.lc_task_responsibility r WHERE r.recommendation_id=:recommendation
                  AND r.first_raised_at<=:at AND r.recorded_at<=:at
                """).param("recommendation",recommendationId).param("at",Timestamp.from(asOf)).query((rs,n)->{
