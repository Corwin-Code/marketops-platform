...
class ListingActionLaunchService implements ListingActionLaunch {
    ...
    public LaunchResult launch(AuthenticatedActor actor, UUID actionId, Map<String, BigDecimal> requestedAxes) {
        ...
        // Match the atomic SQL lock order before computing the execution evidence. Waiting
        // for an action, another launch, or allowance publication must not age an evaluation
        // that was computed before that wait. These transaction locks remain held by SQL.
        ActionContext locked = jdbc.sql("""
                SELECT a.organization_id, a.store_id, a.recommendation_id, b.approval_decision_id, a.state
                  FROM ops.lc_action a
                  LEFT JOIN ops.lc_action_binding b ON b.action_id = a.id
                 WHERE a.id = :id AND a.organization_id = :organization
                 FOR UPDATE OF a
                """).param("id", actionId).param("organization", actor.organizationId())
                .query((rs, n) -> new ActionContext(rs.getObject("organization_id", UUID.class),
                        rs.getObject("store_id", UUID.class), rs.getObject("recommendation_id", UUID.class),
                        rs.getObject("approval_decision_id", UUID.class), rs.getString("state")))
                .optional().orElseThrow(() -> OperationRejectedException.of(ErrorCode.RESOURCE_SCOPE_DENIED));
        if (!context.storeId().equals(locked.storeId())
                || !context.recommendationId().equals(locked.recommendationId())) {
            throw OperationRejectedException.of(ErrorCode.RESOURCE_SCOPE_DENIED);
        }
        context = locked;
        if (context.approvalDecisionId() == null) {
        ...
        jdbc.sql("SELECT ops.lock_lc_launch_evaluation(:action)").param("action", actionId)
                .query((rs, n) -> Boolean.TRUE).single();
        authorization.require(actor, ActionScopeCode.LISTING_ACTION_LAUNCH, ResourceScope.store(context.storeId()));
        if (!actor.stepUpSatisfiedAt(clock.instant())) {
            throw OperationRejectedException.of(ErrorCode.STEP_UP_REQUIRED);
        }
        // The same execution evaluation precedes proof issuance and occupation acquisition.
        var executionEvaluation=guardrails.evaluate(recommendations.require(context.recommendationId()), null,
                com.mimococo.marketops.operationsworkflow.GuardrailPurpose.EXECUTION);
        if (!executionEvaluation.passed()) {
        ...
        String answer = jdbc.sql("""
                SELECT ops.acquire_lc_launch_allowance(:launch, :action, :actor, :proof, CAST(:requested AS jsonb), :executionEvaluation)::text
                """)
                ...
                .param("executionEvaluation",executionEvaluation.evaluationId())
