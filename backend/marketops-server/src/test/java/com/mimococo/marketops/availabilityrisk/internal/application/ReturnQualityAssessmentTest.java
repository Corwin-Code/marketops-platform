...
class ReturnQualityAssessmentTest {
    ...
    void setUp() {
        ...
        evidence(ReturnQualityEvidence.State.FRESH_COMPLETE_OBSERVED_RETURNS);
    }

    @Test
    @DisplayName("TC-RETURN-QUALITY-001 no report evidence is not a zero-return report")
    void absentEvidenceFailsClosed() {
        evidence(ReturnQualityEvidence.State.NO_EVIDENCE);
        ...
        assertThat(answer.blockerCode()).isEqualTo("RETURN_QUALITY_NO_EVIDENCE");
    }

    @Test
    @DisplayName("TC-RETURN-QUALITY-002 fresh complete zero returns is affirmative and clear")
    void freshCompleteZeroReturnsIsClearWithoutReturnEvents() {
        evidence(ReturnQualityEvidence.State.FRESH_COMPLETE_ZERO_RETURNS);
        when(facts.sales(eq(LISTING), eq(SaleStage.COMPLETED), eq(null), any(FactWindow.class)))
                .thenReturn(sales(100));
        when(facts.sales(eq(LISTING), eq(SaleStage.RETAINED), eq(30), any(FactWindow.class)))
                .thenReturn(sales(100));
        when(facts.returns(eq(LISTING), any(FactWindow.class)))
                .thenReturn(ReturnTotals.absent());
    ...
    @Test
    @DisplayName("TC-RETURN-QUALITY-003 stale completed/retained/return/QC report blocks")
    void staleCoverageFailsClosed() {
        evidence(ReturnQualityEvidence.State.STALE);
        assertThat(gatherer.returnQuality(LISTING, POLICY, NOW))
                .isEqualTo(ReturnQualityAssessment.blocked("RETURN_QUALITY_STALE", false));
    }

    @Test
    @DisplayName("TC-RETURN-QUALITY-004 incomplete report coverage blocks")
    void incompleteCoverageFailsClosed() {
        evidence(ReturnQualityEvidence.State.INCOMPLETE);
        assertThat(gatherer.returnQuality(LISTING, POLICY, NOW))
                .isEqualTo(ReturnQualityAssessment.blocked("RETURN_QUALITY_INCOMPLETE", false));
    }

    @Test
    @DisplayName("TC-RETURN-QUALITY-005 conflicted report coverage requires review")
    void conflictedCoverageRequiresReview() {
        evidence(ReturnQualityEvidence.State.CONFLICTED);
        assertThat(gatherer.returnQuality(LISTING, POLICY, NOW))
                .isEqualTo(ReturnQualityAssessment.review("RETURN_QUALITY_CONFLICTED"));
    }

    @Test
    @DisplayName("TC-RETURN-QUALITY-006 defect-heavy returns require supplier or product review")
    ...
    @Test
    @DisplayName("TC-RETURN-QUALITY-007 return and retention breaches cannot remain clear")
    ...
    @Test
    @DisplayName("TC-RETURN-QUALITY-008 fresh complete observed returns inside policy are clear")
    ...
    private void evidence(ReturnQualityEvidence.State state) {
        when(facts.returnQualityEvidence(eq(LISTING), any(FactWindow.class),
                eq(POLICY.evidenceFreshnessMaximum()), eq(NOW)))
                .thenAnswer(invocation -> {
                    FactWindow window = invocation.getArgument(1);
                    return new ReturnQualityEvidence(state, UUID.randomUUID(),
                            window.periodStart(), window.periodEnd(), NOW,
                            "ev://return-quality/report");
                });
    }
